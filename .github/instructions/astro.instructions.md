...
- Add a TSDoc/JSDoc block immediately above every reusable component's `Props` interface that describes the component contract. Document non-obvious prop constraints, defaults, and interactions on the interface properties.
- Keep component documentation focused on the API and intent; do not repeat type information that is already clear from the interface.
